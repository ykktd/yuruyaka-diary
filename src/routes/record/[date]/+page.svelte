<script lang="ts">
	...
	import { Moon, Zap, MapPin, ArrowLeft } from 'lucide-svelte';
...
<div class="mx-auto max-w-md px-4 pt-6 pb-32">
	...
	<div class="mb-8 flex items-center gap-3">
		...
		<div>
			<p class="font-medium text-slate-800">{dateLabel()}</p>
			{#if data.isPastDay}
	...
	<div class="space-y-6">
		...
		{#if data.todayPrompt}
			<div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
				<p class="mb-1 text-xs font-medium tracking-wide text-slate-400 uppercase">
					Question of the day
				</p>
				<p class="mb-3 font-medium text-slate-700">{data.todayPrompt.text}</p>
				<textarea
		...
		<!-- 自由記述エリア -->
		<div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
			<p class="mb-1 text-xs font-medium tracking-wide text-slate-400 uppercase">Free Note</p>
			<textarea
				bind:value={freeNote}
		...
		<!-- 気分スライダー -->
		<div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
			<p class="mb-4 text-xs font-medium tracking-wide text-slate-400 uppercase">Mood</p>
			<MoodSlider bind:value={mood} />
		</div>

		<!-- チェックボックスチップ -->
		<div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
			<p class="mb-4 text-xs font-medium tracking-wide text-slate-400 uppercase">Activities</p>
...
>
	<div class="mx-auto max-w-md">
		...
		<button
			onclick={handleSave}
			disabled={saving}
			class="w-full rounded-xl bg-brand-500 py-4 text-base font-medium text-white shadow-xl shadow-brand-500/20 transition-colors hover:bg-brand-600 disabled:opacity-50"
		>
			{saving ? '保存中...' : '保存する'}
		</button>
	</div>
